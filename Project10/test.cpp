...
protected:
	void SetUp() override {
		...
		SUNDAY_BOOKING = getTime(2021, 3, 28, 17, 0);
		MONDAY_BOOKING = getTime(2024, 6, 3, 17, 0);
...
TEST_F(BookingItem, 현재날짜가일요일인경우예약불가예외처리) {
	//arrange
	BookingScheduler* bookingScheduler = new TestableBookingScheduler{ CAPACITY_PER_HOUR, SUNDAY_BOOKING };
	try {
		//act
		Schedule* schedule = new Schedule{ ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER_WITH_MAIL };
		bookingScheduler->addSchedule(schedule);
...
TEST_F(BookingItem, 현재날짜가일요일이아닌경우예약가능) {
	//arrange
	BookingScheduler* bookingScheduler = new TestableBookingScheduler{ CAPACITY_PER_HOUR , MONDAY_BOOKING };
	//act
	Schedule* schedule = new Schedule{ ON_THE_HOUR, UNDER_CAPACITY, CUSTOMER_WITH_MAIL };
	bookingScheduler->addSchedule(schedule);
	//assert
